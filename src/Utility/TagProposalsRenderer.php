...
OFFSET " . $this->offset);
	...
	public function renderItem(int $index, array $item): void
	{
		...
		echo '<td>';
		echo '<button class="btn" onclick="window.location=\'/tags/acceptTagProposal/' . $item['tag_id'] . '\'" id="tag-accept-' . $item['tag_id'] . '">Accept</button>';
		echo '<button class="btn" onclick="window.location=\'/tags/rejectTagProposal/' . $item['tag_id'] . '\'" id="tag-reject-' . $item['tag_id'] . '">Reject</button>';
		echo '</td>';
	}
}
