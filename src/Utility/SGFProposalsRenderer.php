...
class SGFProposalsRenderer
{
	public function __construct($urlParams)
	{
		...
		$this->count = $this->Sgf->query("SELECT COUNT(DISTINCT tsumego_id) as total FROM sgf")[0][0]['total'];
...
SELECT
    ...
    tsumego_status AS status
    CONCATENATE(`set`.title, ' ', `set`.title2) AS set_title
FROM sgf p
JOIN (
...
OFFSET $offset", [Auth::getUserID()]);
	...
	{
		echo '<h3 style="margin:15px 0;">SGF Proposals (' . $this->proposalsTotal . ')</h3>';
		...
		foreach ($this->toApprove as $toApprove)
		{
			...
			echo '<a href="/editor/?sgfID=' . $toApprove['latest_accepted_id'].'">current</a> |
				...
				<a href="/editor/?sgfID=' . $toApprove['latest_accepted_id'] . '&diffID=' . $toApprove['proposed_id'] . '">diff</a>';
			echo '</td>';
			echo '<td>';
			new TsumegoButton($toApprove['tsumego_id'], $toApprove['set_connection_id'], $toApprove['num'], $toApprove['status'])->render();
			echo '<td><a class="new-button-default2 tag-submit-button" href="/users/adminstats?accept=true&tag_id='
				.$toApprove['proposed_id'] . '">Accept</a>
				<a class="new-button-default2">Reject</a></td>';
			echo '</tr>';
		}
		echo '</table>';
		echo PaginationHelper::render($this->page, $this->pageCount, 'proposals_page');
