 ...
 * state is never stored, only mistake_training_due is persisted.
 *
...
class MistakeTraining
{
	...
	/**
	 * Compute the next mistake_training_due for a user+tsumego pair by replaying the
	 ...
	 * ordered by mistake_training_due (most overdue first).
	 */
	...
	private static function queueSql(): string
	{
		return "
			...
			FROM (
				...
				WHERE ts.user_id = ?
				  AND ts.mistake_training_due IS NOT NULL
				  AND ts.mistake_training_due <= NOW()
				  AND t.deleted IS NULL
			) x
			WHERE rn = 1
			ORDER BY mistake_training_due ASC
		";
	}
}
