...
class TsumegoButton
{
	...
	private function generateTooltip(): string
	{
		...
		$result .= 'black = \'' . implode("", array_map(fn($stone) => BoardPosition::toLetters($stone), $sgf->filterStonesPositions(SgfBoard::BLACK))) . '\';';
		$result .= 'white = \'' . implode("", array_map(fn($stone) => BoardPosition::toLetters($stone), $sgf->filterStonesPositions(SgfBoard::WHITE))) . '\';';
