...
?>
	...
	<div class="homeLeft">
		...
		<?php if(
			...
			|| $this->Session->check('loggedInUserID') && $this->Session->read('loggedInUser.User.premium')==0
		){ ?>
		...
		<?php } ?>
		<p class="title4"<?php if($this->Session->check('loggedInUserID')) echo ' style="margin-top:2px;"'; ?>>Recent Upgrades</p>
		<div class="new1">
			<table class="newx">
				<?php 
		...
		<?php
		if($this->Session->check('loggedInUserID'))
			echo 'The <a href="/users/view/'.$this->Session->read('loggedInUserID').'">profile page</a> contains more data and is organized in line and bar charts.';
		else
	...
	<script>
		...
		let currentLeft = highestLeft - 4;
		let currentRight = highestRight - 5;

		$("#show-more-left").click(function(){
			currentLeft -= 3;
			for(let i=highestLeft;i>currentLeft;i--)
				if(i>1)
					$("#news-left-"+i).fadeIn(250);
				else
					$("#show-more-left").hide();
		});

		$("#show-more-right").click(function(){
			currentRight -= 3;
			for(let i=highestRight;i>currentRight;i--)
				if(i>1)
					$("#news-right-"+i).fadeIn(250);
				else
