...
<?php
	if(!$this->Session->check('loggedInUserID') || $this->Session->read('loggedInUser.User.isAdmin')==0)
		...
		if($approveSgfs!=null){
			...
			for($i=0; $i<count($approveSgfs); $i++){
				echo '<tr>';
					...
					.$approveSgfs[$i]['Sgf']['id'].'&hash='.md5($this->Session->read('loggedInUserID')).'">Submit (1)</a>
		...
		if($tagNames!=null){
			...
			for($i=0; $i<count($tagNames); $i++){
				echo '<tr>';
					...
					echo '<td>';
					if($this->Session->read('loggedInUserID') != $tags[$i]['Tag']['user_id']){
						...
						.$tagNames[$i]['TagName']['id'].'&hash='.md5($this->Session->read('loggedInUserID')).'">Submit (1)</a>
			...
			for($i=0; $i<count($tags); $i++){
				echo '<tr>';
					...
					if($this->Session->read('loggedInUserID') != $tags[$i]['Tag']['user_id']){
						...
						.$tags[$i]['Tag']['id'].'&hash='.md5($this->Session->read('loggedInUserID')).'">Submit</a>';
					}
					echo '</td>';
