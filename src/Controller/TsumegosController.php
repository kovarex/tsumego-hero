	...
	public function mistakeTraining()
	{
		...
		$next = ClassRegistry::init('TsumegoStatus')->find('first', [
			'conditions' => [
				...
				'mistake_training_due <=' => date('Y-m-d H:i:s'),
				'mistake_training_due IS NOT NULL',
			],
			'order' => 'mistake_training_due ASC',
		]);
		...
		if (!$next)
		{
			...
			{
				$day = date('Y-m-d', strtotime($row['TsumegoStatus']['mistake_training_due']));
