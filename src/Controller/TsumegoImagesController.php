...
class TsumegoImagesController extends AppController
{
	...
	public function tsumegoImage($setConnectionId = null)
	{
		...
		$this->response->modified(strtotime($tsumego['Tsumego']['modified'] ?? 'now'));
	...
	private function _generatePuzzleImage($sgfString, $setTitle, $description)
	{
		...
		foreach ($starPoints as $point)
		{
			if ($point[0] >= $minX && $point[0] <= $maxX && $point[1] >= $minY && $point[1] <= $maxY)
			{
				...
				imagefilledellipse($img, $x, $y, $cellSize * 0.2, $cellSize * 0.2, $lineColor);
			}
		}
		...
		foreach ($stones as $stone)
		{
			...
			if ($stone['color'] === 'B')
			{
				...
				imagefilledellipse($img, $x, $y, $stoneRadius * 2, $stoneRadius * 2, $blackStone);
			}
			else
			{
				// White stone with black border
				imagefilledellipse($img, $x, $y, $stoneRadius * 2, $stoneRadius * 2, $whiteStone);
				imageellipse($img, $x, $y, $stoneRadius * 2, $stoneRadius * 2, $lineColor);
			}
		}
