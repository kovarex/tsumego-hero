...
# Ensure shared directory structure exists (idempotent — safe to run every startup)
...
# Create core.local.php if not present (Docker bind mounts cause PHP require to hang on WSL2)
if [ ! -f "$SHARED_CONFIG/core.local.php" ]; then
    cat > "$SHARED_CONFIG/core.local.php" << 'COREEOF'
<?php
