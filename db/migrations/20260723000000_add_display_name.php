...
final class AddDisplayName extends AbstractMigration
{
	 ...
	 * - Users with the same normalized email (g__ prefix stripped) are the same
	 *   person; keep the account they actually play on (active use), else the MAIN
	 *   account (the one with the MOST attempts).
	 * - An account is "alive" if it has >= 200 attempts OR had any activity in the
	 *   last 180 days.
	 * - If any survivor is alive, keep the OLDEST (lowest id) alive one (seniority).
	...
	private function pickKeptUserId(array $ids): int
	{
		...
		// Same normalized email = same person -> keep the account they actually
		// play on (if one is actively used), else the MAIN account (most attempts).
		$clusters = [];
		foreach ($candidates as $id)
			$clusters[$this->normalizeEmail($this->emailOf($id))][] = $id;

		$survivors = [];
		foreach ($clusters as $email => $clusterIds)
		{
			$real = array_filter($clusterIds, fn($id) => $this->attemptCount($id) >= self::REAL_ATTEMPTS);
			$pool = $real ? array_values($real) : $clusterIds;
		...
		return $this->pickAmongSurvivors($survivors);
	}
