...
class SetsControllerTest extends TestCaseWithAuth
{
	...
	{
		$collectionTopDivs = $browser->getCssSelect('#playTitle');
		$this->assertCount(1, $collectionTopDivs);
	...
	public function testFullProcessOfDifficultyBasedSelectionAndSolving(): void
	{
		...
		$browser->clickId('tile-difficulty-submit');
	...
	public function testFullProcessOfPartitionedSetBasedSelection(): void
	{
		...
		$browser->get('sets');
		$collectionTopDivs = $browser->getCssSelectSafe('.collection-top', 2);
		$this->assertCount(2, $collectionTopDivs); // 2 partitions of 2 problems
	...
	public function testOfVisiting2RankBasedSetsBothInTheFilters(): void
	{
		...
		$collectionTopDivs[0]->click();
		$this->assertSame(Util::getMyAddress() . '/sets/view/15k', $browser->getCurrentURL());
		$this->assertSame($browser->getCssSelect('.title4')[1]->getText(), '15k');
		...
		$this->assertSame($browser->getCssSelect('.title4')[1]->getText(), '1d');
	...
	public function testQueringSetsByTopicButLimitedByRanks(): void
	{
		...
		// all 3 sets with
		$browser->get("sets");
		$collectionTopDivs = $browser->getCssSelectSafe('.collection-top', 3);
		...
		$collectionMiddleLeftDivs = $browser->getCssSelectSafe('.collection-middle-left', 3);
		...
		$this->assertSame($browser->getCssSelectSafe('.title4', 2)[1]->getText(), 'set 1');
	...
	public function testSelectingTagFilters(): void
	{
		...
		// first we select the difficulty of 15k
		$browser->get("sets");
		$browser->clickId('tags-button');
		$tagSelectors = $browser->getCssSelectSafe('[id^="tile-tags"]:not([id*="select-all"]):not([id*="submit"])', 3);
		...
		$browser->clickId('tile-tags-submit');
	...
	public function testVisitingTagBasedSetsRespectsTagFilters(): void
	{
		...
		$this->assertSame($browser->getCssSelect('.title4')[1]->getText(), 'atari');
	...
	public function testVisitingTopicBasedSetsRespectsTagFilters(): void
	{
		...
		$this->assertSame('atari@empty triangle', ClassRegistry::init('UserContribution')->find('first', ['conditions' => ['user_id' => $context->user['id']]])['UserContribution']['filtered_tags']);
		$this->assertSame('topics', ClassRegistry::init('UserContribution')->find('first', ['conditions' => ['user_id' => $context->user['id']]])['UserContribution']['query']);
		// now we are viewing the 'set 1' insides and checking the buttons
	...
	public function testAddingToFavoritesAndViewingIt(): void
	{
		...
		$this->assertSame($browser->getCssSelect('.title4')[1]->getText(), 'Favorites');
	}
	...
	public function testOnlyPrivateSetsAreVisibleInSandbox(): void
	{
		...
		$problemButtons = $browser->getCssSelectSafe('.setViewButtons1');
		$this->assertCount(1, $problemButtons);
	...
	public function testAddingProblemInSandbox(): void
	{
		...
		$problemButtons = $browser->getCssSelectSafe('.setViewButtons1', 2);
	...
	public function testSetViewCompletedTabShowsOrderNumbers()
	{
		...
		$buttons = $browser->getCssSelectSafe('.setViewButtons1', 3);
	...
	public function testSetViewAccuracyTabShowsSuccessFailureRatio()
	{
		...
		$accuracyButtons = $browser->getCssSelectSafe('.setViewButtons2', 2);
		...
		$orderButtons = $browser->getCssSelect('.setViewButtons1');
