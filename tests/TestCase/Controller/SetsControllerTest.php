...
class SetsControllerTest extends TestCaseWithAuth
{
	...
	public function testVisitingTopicBasedSetsRespectsTagFilters(): void
	{
		...
		$this->assertSame('atari@empty triangle', ClassRegistry::init('UserContribution')->findById($context->user['id'])['UserContribution']['filtered_tags']);
		$this->assertSame('topics', ClassRegistry::init('UserContribution')->findById($context->user['id'])['UserContribution']['query']);
		// now we are viewing the 'set 1' insides and checking the buttons
