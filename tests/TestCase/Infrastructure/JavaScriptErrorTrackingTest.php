...
/**
 * Meta-test to ensure JavaScript error tracking infrastructure is working.
 * This test verifies that debug mode is enabled in test environment and that
 * window.__jsErrors tracking is properly initialized.
 *
 * If this test fails, it means other browser tests won't catch JS errors.
 */
class JavaScriptErrorTrackingTest extends CakeTestCase
{
	...
	public function testErrorTrackingIsEnabled()
	{
		...
		$this->assertNotNull(
			$jsErrors,
			'window.__jsErrors must exist! If null, debug mode is disabled and JS error tracking is BROKEN.'
		);
		...
		$this->assertNotNull(
			$consoleErrors,
			'window.__consoleErrors must exist! If null, debug mode is disabled and error tracking is BROKEN.'
		);
