					...
					<tr>
						{canDrag && (
							<td className="tsumego-comment__drag-handle-cell">
								<span className="tsumego-comment__drag-handle" title="Drag to move comment">☰</span>
							</td>
						)}
						<td>
						...
						<td align="right" className="sandboxTable2time">
							...
							{comment.position && (
								<img 
									...
									onClick={() => {
										if (!window.besogo?.editor) return;
							...
							{canDelete && <button className="deleteComment" onClick={() => onDelete(comment.id)}>Delete</button>}
							{canMakeIssue && <button className="tsumego-comment__make-issue-btn" onClick={() => onMakeIssue(comment.id)}>📋 Make Issue</button>}
						</td>
					</tr>
				</table>
			</div>
